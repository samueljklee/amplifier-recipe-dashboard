...
class RecipeDashboard {
    ...
    async _showDetail(sessionId, isPoll = false) {
        ...
        const childSessionsHtml = this._renderChildSessions(session);
        ...
        // Child sessions (sub-recipes) -- show prominently when present
        if (childSessionsHtml) {
            html += this._renderCollapsibleSection(
                'child-sessions', 'Sub-Recipe Sessions', childSessionsHtml,
                false  // expanded by default -- this is key info
            );
        }
